...
public class TestPylonZombie extends TestCase{
	...
	public void setUp() throws Exception {
		...
		isEaten = false; 
		pz1 = new PylonZombie(10, 10, isEaten, damagePoints, 'x' );
		pz2 = new PylonZombie(20, 20, isEaten, damagePoints, 'x');
		pz3 = new PylonZombie(30, 30, isEaten, damagePoints, 'x');
		...
		for(PylonZombie pz: pylonZombies ) {
			for (Plant p: plants) {
				...
				if(plantType == 'p' && pz.getPositionX() == p.getPositionX()) {
					damagePoints -= 100;
					assertEquals("The damage points are ", damagePoints, PylonZombie.getDmg());	
				}
				 // check cherrybomb plant
				if(plantType == 'c' && (p.getPositionX() - pz.getPositionX() == 1 || p.getPositionY() - pz.getPositionY() <= 1)) {
					damagePoints = 0;
					assertEquals("The damage points are 0", damagePoints, PylonZombie.getDmg());	
				}
				//check walnut plant
				if(plantType == 'w' && (p.getPositionX() - pz.getPositionX() == 1 || p.getPositionY() - pz.getPositionY() <= 1)) {
					damagePoints = 0;
					assertEquals("The damage points are 0", damagePoints, PylonZombie.getDmg());	
				}
			}	
